...
def run(input_data: str):
    ...
    for idx, _ in enumerate(input_data):
        if (
            packet_start is None
            and len(set(input_data[idx : idx + PACKET_MARK_SIZE])) == PACKET_MARK_SIZE
        ):
            packet_start = idx + PACKET_MARK_SIZE

        if (
            message_start is None
            and len(set(input_data[idx : idx + MESSAGE_MARK_SIZE])) == MESSAGE_MARK_SIZE
        ):
